...
# Blocks git commits containing secrets, legacy paths, or dart analyze failures
...
if echo "$COMMAND" | grep -q "^git commit"; then
  ...
  if [ -n "$STAGED_FILES" ]; then
    ...
    if [ -n "$SECRET_HITS" ]; then
      echo "{\"continue\": false, \"stopReason\": \"Potential secret detected in staged files: $SECRET_HITS — remove before committing.\"}"
      exit 0
    fi
  fi
fi
...
if echo "$COMMAND" | grep -q "^git commit"; then
  if [ -d "lib" ]; then
    STAGED_DART=$(git diff --cached --name-only --diff-filter=ACMR | grep "^lib/.*\.dart$")
    if [ -n "$STAGED_DART" ]; then
      ANALYSIS=$(dart analyze lib/ --no-fatal-infos 2>&1)
      EXIT_CODE=$?
