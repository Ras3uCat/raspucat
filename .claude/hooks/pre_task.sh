...
if grep -qi "stripe\|payment" "$ACTIVE_FILE"; then
  echo "⚖️ ARCHITECT ADVISORY: Task involves Payments. Ensure .claude/skills/payments_dev/ is loaded."
fi

if grep -qi "flutter\|widget\|screen" "$ACTIVE_FILE"; then
...
if grep -qi "supabase\|migration\|rls\|repository" "$ACTIVE_FILE"; then
  echo "🗄️ ARCHITECT ADVISORY: Backend task. Ensure .claude/skills/backend-dev/ is loaded."
fi
