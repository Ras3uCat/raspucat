# 042 — Lead-to-Client Conversion Skill
...
## Overview
The `admin-convert-lead-to-quote` edge function exists and the "Convert to Client" bottom sheet
is implemented in the Flutter admin panel. But there is no skill documenting when to use it,
what inputs are required, what outputs to expect, or what happens in the portal after conversion.

Anyone running the outreach pipeline for the first time will stall at the "Convert to Client"
stage because the skill chain ends at `prepare-reply` with a vague "What Comes Next" note.

---

## User Stories
- As the business owner, I want a skill that tells me exactly when and how to convert a lead
  to a client so the handoff to the build pipeline is smooth and nothing falls through the cracks.

---

## Acceptance Criteria
- [ ] `.claude/skills/convert-lead/SKILL.md` exists with YAML frontmatter (name, description, trigger phrases).
- [ ] Skill covers: trigger conditions, all required inputs, UI steps, expected outputs, and what admin sees next.
- [ ] Skill links to `prepare-lead` (pipeline start), `prepare-reply` (upstream), and `client-delivery` (downstream).
- [ ] Skill documents what to do if the client wants to negotiate price after the proposal.
- [ ] Skill documents the edge case where lead was never prepared (no reports) — can still convert.
- [ ] Skill documents the "no email address" edge case and how to resolve it before converting.
- [ ] Skill documents magic link expiry (24h) and how to send a new login link after it expires.
- [ ] Skill documents what to do if conversion fails mid-flight (orphan quote with no deposit URL).
- [ ] Skill documents the re-convert warning (check for existing quote before converting a second time).
- [ ] Skill documents silent email failure and how to verify the client received the conversion email.
- [ ] Skill documents all bottom-sheet inputs including `billingCycle` and `managementOptionId`.
- [ ] AC verified: Flutter bottom sheet exposes `billingCycle` and `managementOptionId` fields
      (or skill notes they are not yet exposed in UI).

---

## Scope Control
- [x] Included: Skill documentation only — no code changes
- [x] NOT Included: Changes to the Convert to Client UI or edge function behavior

---

## Implementation Detail

**New file:** `.claude/skills/convert-lead/SKILL.md`
...
## Edge Cases & QA
- [ ] Skill frontmatter includes trigger phrases that match real-world language ("they want to move forward", "close [company]").
- [ ] Skill links to `prepare-lead` at the top (pipeline start).
- [ ] Skill links to `prepare-reply` at the top (upstream).
- [ ] Skill links to `client-delivery` at the bottom (downstream).
- [ ] Skill registered in `.claude/skills/convert-lead/SKILL.md` format consistent with other skills.
- [ ] Flutter bottom sheet fields verified against edge function accepted inputs.
