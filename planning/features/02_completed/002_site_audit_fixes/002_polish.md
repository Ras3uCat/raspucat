...
### SEO — `web/index.html`
- [x] `<title>` is descriptive (e.g., `Ras3uCat — Flutter & Mobile Developer`).
- [x] `<meta name="description">` has a meaningful value proposition (not just `"Ras3uCat"`).
- [x] `<meta property="og:title">` is present.
- [x] `<meta property="og:description">` is present.
- [x] `<meta property="og:image">` is present (use site logo or a social card asset).
- [x] `<meta name="theme-color">` is set to `#000612`.
- [x] `<meta name="author">` is present.

### Manifest — `web/manifest.json`
- [x] `"orientation"` is `"any"`.
- [x] `"description"` matches the HTML meta description.

### pubspec — `pubspec.yaml`
- [x] `description` field is no longer `"A new Flutter project."`.

### 404 Screen — `main.dart` unknownRoute
- [x] Unknown routes render a dedicated `NotFoundScreen` widget.
- [x] `NotFoundScreen` uses brand colors (`#000612` bg, `#58E3EF` text).
- [x] `NotFoundScreen` includes a button that navigates back to `ERoutes.home`.
- [x] `NotFoundScreen` text uses `EBrand` or `EStrings` constants, not hardcoded strings.

### Constants consistency — `main.dart`
- [x] `title:` in `GetMaterialApp` uses `EBrand.appName`.

### Project data — `project_data.dart`
- [x] The commented-out Dashing Beard Co GIF is either restored, replaced, or the comment
