...
- [x] `isMacOS()` returns `GetPlatform.isMacOS`.
- [x] `isWindows()` returns `GetPlatform.isWindows`.
- [x] `isLinux()` returns `GetPlatform.isLinux`.
- [ ] Manual test: running on Linux confirms `isLinux()` returns `true`.

### Bug 2: GitHub Button (`project_screen.dart:86-87`)
- [x] Tapping the GitHub button opens `project.githubUrl` in the browser.
- [x] Button is not rendered if `project.githubUrl` is null (existing guard retained).
- [x] `EDeviceUtils.launchUrl()` is used (not a raw `url_launcher` call).

### Bug 3: Print Statement (`responsive_layout.dart:20`)
- [x] `print(constraints.maxWidth)` line is deleted.
- [x] No `print()` calls remain outside of `kDebugMode` guards in the codebase.
